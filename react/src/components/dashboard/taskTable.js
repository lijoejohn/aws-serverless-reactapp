...
import { showLoader, getTaskList, updateTask, deleteTask, editTask } from "../../store/actions";
...
const TaskTable = ({ searchTeram }) => {
	...
	const onEditTask = ({ id }) => {
		const data = { id: id, name: "Refactor something updated", completed: true };
					...
					{list?.map(({ name, completed, id }) => (
						<Col key={id} className="box table-row-item" lg={12} md={12} sm={12} xs={12}>
							...
							<div className="icon-group">
								...
								/>
								<img
									onClick={() => onEditTask({ id, show: true })}
									className="icon"
									alt="icon"
									src={editIcon}
								/>
